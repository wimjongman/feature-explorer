...
public class FeatureExplorer extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		text.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				if (text.getText().length() > 0)
					treeViewer
							.setFilters(new ViewerFilter[] { new FeatureFilter(
									text.getText()) });
				else
		...
		tcl_composite.setColumnData(trclmnFeature, new ColumnWeightData(2, 100,
				true));
		...
		tcl_composite.setColumnData(trclmnVersion, new ColumnWeightData(1,
		...
		tcl_composite.setColumnData(trclmnState, new ColumnWeightData(1,
				ColumnWeightData.MINIMUM_WIDTH, true));
		...
		tcl_composite.setColumnData(trclmnProvider, new ColumnWeightData(1,
				ColumnWeightData.MINIMUM_WIDTH, true));
		trclmnProvider.setText("Provider");

		TreeViewerColumn treeViewerColumn_4 = new TreeViewerColumn(treeViewer,
				SWT.NONE);
		treeViewerColumn_4
				.setLabelProvider(new DescriptionColumnLabelProvider());
		TreeColumn trclmnDescription = treeViewerColumn_4.getColumn();
		tcl_composite.setColumnData(trclmnDescription, new ColumnWeightData(3,
				ColumnWeightData.MINIMUM_WIDTH, true));
